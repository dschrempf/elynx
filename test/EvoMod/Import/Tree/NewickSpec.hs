{- |
Module      :  EvoMod.Import.Tree.NewickSpec
...
spec = do
  describe "branchLength" $ do
    it "parses a colon and a branch length" $
      parse branchLength "" (T.pack ":13.2") `shouldParse` 13.2

    it "returns 0 if no branch length is given" $
      parse branchLength "" (T.pack "") `shouldParse` 0

  describe "name" $ do
    it "parses a string of printable characters" $
      parse name "" (T.pack "aName") `shouldParse` T.pack "aName"

    it "parses blanks, colons, semicolons, parentheses, and sequare brackets" $
      parse name "" (T.pack "aName bla") `shouldParse` T.pack "aName"

    it "allows empty names" $
      parse name "" (T.pack "") `shouldParse` T.pack ""

  describe "node" $ do
    it "parses a tree node" $
      parse node "" sampleLabelText `shouldParse` sampleLabel

    it "parses tree nodes with empty names and branch lengths" $
      parse node "" (T.pack "") `shouldParse` PhyloLabel (T.pack "") 0

  describe "leaf" $
    it "parses a leaf of a tree" $
      parse leaf "" sampleLabelText `shouldParse` Node sampleLabel []

  describe "forest" $
    it "parses a set of trees within brackets" $
    parse forest "" sampleForestText `shouldParse` sampleForest

  describe "newick" $ do
    it "parses a newick trees" $ do
      parse newick "" `shouldSucceedOn` sampleNewickText1
      parse newick "" `shouldSucceedOn` sampleNewickText2

    it "parses a weird newick tree without node labels nor branch lengths" $
      parse newick "" sampleNewickEmptyText `shouldParse` sampleNewickEmpty
