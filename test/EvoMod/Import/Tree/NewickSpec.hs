...
sampleNodeText :: T.Text
sampleNodeText = T.pack "name:0.3"

sampleNode :: PhyloTextNode
sampleNode = PhyloNode (T.pack "name") 0.3
...
sampleForest =
  [ Node { rootLabel = PhyloNode {nLabel = T.pack "l", nBranchLength = 0.0}
         , subForest = []
         }
  , Node { rootLabel = PhyloNode {nLabel = T.pack "l", nBranchLength = 0.0}
         , subForest = []
         }
  , Node { rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}
         , subForest =
           [ Node {rootLabel = PhyloNode {nLabel = T.pack "a", nBranchLength = 0.0}, subForest = []}
           , Node {rootLabel = PhyloNode {nLabel = T.pack "b", nBranchLength = 0.0}, subForest = []}
           ]
         }
  ]
...
sampleNewickEmpty =
  Node { rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}
       , subForest =
           [ Node {rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}, subForest = []}
           , Node {rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}, subForest =
                      [ Node {rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}, subForest = []}
                      , Node {rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}, subForest = []}
                      , Node {rootLabel = PhyloNode {nLabel = T.pack "", nBranchLength = 0.0}, subForest = []}
...
spec = do
  ...
  describe "node" $ do
    it "parses a tree node" $
      parse node "" sampleNodeText `shouldParse` sampleNode

    it "parses tree nodes with empty names and branch lengths" $
      parse node "" (T.pack "") `shouldParse` PhyloNode (T.pack "") 0

  describe "leaf" $
    it "parses a leaf of a tree" $
      parse leaf "" sampleNodeText `shouldParse` Node sampleNode []
  ...
  describe "newick" $ do
    it "parses a newick tree" $
      parse newick "" `shouldSucceedOn` sampleNewickText
