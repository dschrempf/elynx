...
sampleLabelByteString :: B.ByteString
...
spec = do
  describe "branchLength" $ do
    it "parses a colon and a branch length" $
      parse branchLength "" (B.pack ":13.2") `shouldParse` 13.2

    it "returns 0 if no branch length is given" $
      parse branchLength "" (B.pack "") `shouldParse` 0

  describe "name" $ do
    it "parses a string of printable characters" $
      parse name "" (B.pack "aName") `shouldParse` B.pack "aName"

    it "parses blanks, colons, semicolons, parentheses, and sequare brackets" $
      parse name "" (B.pack "aName bla") `shouldParse` B.pack "aName"

    it "allows empty names" $
      parse name "" (B.pack "") `shouldParse` B.pack ""
