...
prop_connect n l r
        |
  -- XXX: Skip not bifurcating trees. This is ugly, I know.
          not (bifurcating l) || not (bifurcating r)
        = True
        | length (leaves l) < 3 || length (leaves r) < 3
        = length (connect n l r) == 1
        | otherwise
        = length (connect n l r) == length (leaves l) * length (leaves r)

type Constraint a = S.Set a

compatibleAll :: (Show a, Ord a) => Tree a -> [Constraint a] -> Bool
compatibleAll (Node _ [l, r]) cs =
        all (compatible (bipartition l)) cs
...
spec = do
        ...
        describe "connect" $ modifyMaxSize (* 100) $ do
                ...
                it
                                "correctly connects sample trees without and with constraints"
                        $ do
                                  a <- parseFileWith
                                          oneNewick
                                          "data/ConnectA.tree"
                                  ...
                                  let     ts = connect
                                                  ...
                                                  b
                                          cs =
                                                  concatMap clades c :: [ Constraint
                                                                    ( PhyloLabel
                                                                              L.ByteString
                                                                    )
                                                          ]
                                          ts' = filter
                                                  (compatibleWith getName cs)
                                                  ts
