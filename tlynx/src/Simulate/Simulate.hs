...
import           ELynx.Simulate.PointProcess          (TimeSpec, simulateNReconstructedTrees,
...
simulate a@(SimulateArguments nTrees nLeaves height mrca lambda mu rho subS sumS seed) = do
  ...
  trs <- if subS
         then simulateAndSubSampleNTreesConcurrently nLeaves lambda mu rho timeSpec chunks gs
...
simulateNTreesConcurrently :: Int -> Double -> Double -> Double
                           -> TimeSpec
                           -> [Int] -> [GenIO]
...
simulateAndSubSampleNTreesConcurrently :: Int -> Double -> Double -> Double
                                       -> TimeSpec
                                       -> [Int] -> [GenIO]
                                       -> ELynx [Tree (PhyloLabel Int)]
simulateAndSubSampleNTreesConcurrently nLeaves l m r timeSpec chunks gs = do
  ...
  logNewSection $ T.pack $ "Sub sample " <> show (sum chunks) <> " trees with " <> show nLeaves <> " leaves."
