...
reportCoalesceArguments a =
  intercalate "\n" [ "Number of simulated trees: " ++ show (argsNTrees a)
                   ...
                   , "Seed: " ++ sStr ]
  where sStr = case argsSeed a of Nothing -> "Random"
                                  Just i  -> show i
...
coalesceArguments = CoalesceArguments
  <$> nTreeOpt
  <*> nLeavesOpt
  <*> rhoOpt
  <*> sumStatOpt
...
rhoOpt = optional $ option auto $
  long "sub-sample"
  <> short 'r'
  <> metavar "DOUBLE"
  <> showDefault
  <> help "Perform random sub-sampling with sampling probability rho (see below)"
