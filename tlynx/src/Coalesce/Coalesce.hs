...
simulateNTreesConcurrently (CoalesceArguments nT nL _ _ (Fixed s)) = do
  c  <- liftIO getNumCapabilities
  ...
  trss <- liftIO $ mapConcurrently (\(n, g) -> replicateM n $ simulate nL g)
                                   (zip chunks gs)
  return $ concat trss
simulateNTreesConcurrently _ = error
  "simulateNTreesConcurrently: seed has not been set; contact maintainer."

simulateAndSubSampleNTreesConcurrently
  :: CoalesceArguments -> ELynx [Tree (PhyloLabel Int)]
simulateAndSubSampleNTreesConcurrently (CoalesceArguments nT nL mR _ (Fixed s))
  = do
    c <- liftIO getNumCapabilities
    let
      r = fromMaybe
...
simulateAndSubSampleNTreesConcurrently _ =
  error
    "simulateAndSubSampleNTreesConcurrently: seed has not been set; contact maintainer."
